...
export default function FolderTreeItem({
  ...
  const rowCls = `w-full flex items-center justify-between rounded-lg text-sm transition-colors ${
    isActive
      ...
      : 'text-gray-500 hover:text-gray-700 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-[#1a2030]'
  }`;
      ...
      <div className="relative group px-2" style={{ paddingLeft: 8 + depth * 12 }}>
        {editingId === node.id ? (
          ...
          <input
            ...
            className="w-full px-2 py-1.5 text-sm bg-gray-100 dark:bg-[#1a2030] border border-yellow-500/50 rounded-lg text-gray-900 dark:text-white focus:outline-none my-0.5"
          />
        ) : (
          ...
          >
            ...
            <span className="flex items-center gap-1 shrink-0">
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-gray-200 dark:hover:bg-[#252d3d] transition-opacity"
              >
        ...
        {menuId === node.id && (
          ...
          >
            <button type="button" onClick={() => { setEditingId(node.id); setEditingName(node.name); setMenuId(null); }}
              className="w-full px-3 py-2 text-sm text-left text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#1a2030]">
              Renommer
            </button>
            ...
            <button type="button" onClick={() => { onDeleteFolder(node.id); setMenuId(null); }}
              className="w-full px-3 py-2 text-sm text-left text-red-500 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-[#1a2030]">
              Supprimer
            </button>
          </div>
        )}
