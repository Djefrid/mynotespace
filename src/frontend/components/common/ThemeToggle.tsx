...
export default function ThemeToggle({ variant = 'icon', className = '' }: ThemeToggleProps) {
  ...
  if (!mounted) {
    ...
    if (variant === 'segmented') return <div className="h-9 w-56 rounded-lg bg-gray-100 dark:bg-dark-650 animate-pulse" />;
  ...
  return (
    <div
      ...
      className={`inline-flex gap-1 p-1 rounded-xl bg-gray-100 dark:bg-dark-650 ${className}`}
    >
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
              active
                ? 'bg-white dark:bg-dark-650 text-primary-600 dark:text-primary-400 shadow-sm'
